...
export function AiResearchWatchPanel({
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || 'Gagal memuat pantauan');
      setWatches(Array.isArray(payload.watches) ? payload.watches : []);
      if (typeof payload.limit === 'number') setLimit(payload.limit);
      setError('');
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Gagal memuat pantauan');
    } finally {
      setLoading(false);
    }
  };
  ...
  const createWatch = async () => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || 'Gagal membuat pantauan');
      setTopic('');
      setKeywords('');
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Gagal membuat pantauan');
    } finally {
      setBusy('');
    }
  };
  ...
  const patchWatch = async (id: string, body: Record<string, unknown>, kind: 'check' | 'status') => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || 'Gagal memperbarui pantauan');
      if (payload.watch) {
        setWatches(current => current.map(item => item.id === id ? payload.watch as WatchItem : item));
      }
      if (payload.message) setError(payload.message);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Gagal memperbarui pantauan');
    } finally {
      setBusy('');
    }
  };
  ...
  const removeWatch = async (id: string) => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || 'Gagal menghapus pantauan');
      setDeleteArmed('');
      setWatches(current => current.filter(item => item.id !== id));
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Gagal menghapus pantauan');
    } finally {
      setBusy('');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end" data-testid="ai-research-watch-panel">
      <button type="button" aria-label="Tutup pantauan" className="absolute inset-0 bg-black/45" onClick={onClose} />
      <aside className="relative flex h-full w-[min(100vw,24rem)] flex-col border-l border-[var(--mos-border)] bg-[var(--mos-bg)] shadow-2xl">
        <div className="flex items-start justify-between gap-2 border-b border-[var(--mos-border)] px-4 py-3">
          <div>
            <h2 className="text-sm font-semibold text-[var(--mos-text)]">Pantauan topik</h2>
            <p className="mt-0.5 text-[11px] text-[var(--mos-text-muted)]">{watches.length}/{limit} pantauan · cek harian atau sekarang</p>
          </div>
        ...
        >
          <input
            ...
            placeholder="Topik, misalnya emas atau nama kompetitor"
            aria-label="Topik pantauan"
            maxLength={120}
          ...
          <input
            ...
            placeholder="Kata kunci tambahan, pisahkan koma"
            aria-label="Kata kunci pantauan"
            className="w-full rounded-lg border border-[var(--mos-border)] bg-[var(--mos-raised)] px-2.5 py-1.5 text-xs text-[var(--mos-text)] outline-none focus:border-indigo-400/60"
          />
          <button
            type="submit"
          ...
          >
            {busy === 'create' ? 'Menyimpan…' : 'Pantau topik'}
          </button>
        </form>
        {error && <p className="px-4 pt-2 text-[11px] text-amber-200">{error}</p>}
        <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
          {loading && watches.length === 0 && <p className="text-[11px] text-[var(--mos-text-muted)]">Memuat pantauan…</p>}
          {!loading && watches.length === 0 && (
            <p className="text-[11px] leading-5 text-[var(--mos-text-muted)]">Belum ada pantauan. Tambahkan topik seperti emas, Rupiah, atau nama kompetitor.</p>
          )}
          {watches.map(watch => (
            <article key={watch.id} className="rounded-xl border border-[var(--mos-border)] bg-[var(--mos-raised)] p-3" data-testid="ai-research-watch-card">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-[10px] text-[var(--mos-text-muted)]">
                    {watch.status === 'active' ? 'Aktif' : 'Dijeda'}
              ...
              <p className="mt-2 max-h-36 overflow-y-auto whitespace-pre-wrap text-[11px] leading-4 text-[var(--mos-text-secondary)]">
                {watch.lastDigest || 'Belum ada ringkasan. Tekan Cek sekarang.'}
              </p>
              {watch.lastCheckedAt && (
                <p className="mt-1 text-[10px] text-[var(--mos-text-faint)]">
                  Dicek {new Date(watch.lastCheckedAt).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}
                </p>
              )}
              <div className="mt-2 flex flex-wrap gap-1.5">
                ...
                >
                  {busy === `${watch.id}:check` ? 'Mengecek…' : 'Cek sekarang'}
                </button>
                <button
                  type="button"
                ...
                >
                  {watch.status === 'active' ? 'Jeda' : 'Lanjutkan'}
                </button>

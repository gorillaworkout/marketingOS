...
export function buildEventPlanDownload(input: EventPlanDownloadInput, format: 'doc' | 'json') {
  ...
  const budgetRows = items.map((item) => `<tr><td>${escapeHtml(item.category || 'Other')}</td><td>${escapeHtml(item.suggestedVendor || '—')}<br><span>${escapeHtml(item.venue || '')}</span></td><td>${escapeHtml(item.notes || '—')}</td><td class="amount">${idr(item.estimatedCost)}</td></tr>`).join('') || '<tr><td colspan="4">No itemized budget details were provided.</td></tr>';

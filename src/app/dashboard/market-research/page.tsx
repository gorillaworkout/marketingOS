...
export default function MarketResearchPage() {
  ...
  return (
    <PageStack>
      <PageHeader eyebrow="Create / Market intelligence" title="Market research" description="Give a brief. MarketingOS scans Forex majors, Commodity (XAUUSD/WTI), US Indices, and US Stocks separately, then selects up to ten High Importance stories published today. Each symbol is covered by one article." actions={<Link href="/dashboard/history?type=market-research" className="inline-flex h-9 items-center rounded-[var(--mos-radius-control)] border border-[var(--mos-border)] bg-[var(--mos-raised)] px-3.5 text-sm font-medium text-[var(--mos-text-secondary)] hover:border-[var(--mos-border-strong)]">Open history</Link>} />
      <InlineModelSelector feature="market-research" />

      <Panel>
        <SectionHeader title="Research brief" description="The research date is locked to today in WIB." action={<Button size="sm" onClick={fillExample}>Fill example</Button>} />
        <div className="mt-5 grid gap-4 md:grid-cols-[1fr_220px]">
          <FormField label="Brief" hint={`${brief.trim().length}/2000 characters`}><TextArea value={brief} onChange={event => setBrief(event.target.value)} rows={6} maxLength={2000} placeholder="Example: Find the most important factual developments for today's morning briefing…" /></FormField>
          <FormField label="Research date" hint="WIB"><TextInput type="date" value={researchDate} readOnly /></FormField>
        </div>
        <div className="mt-4"><p className="text-xs font-semibold uppercase tracking-wider text-[var(--mos-text-faint)]">Product groups always scanned</p><div className="mt-2 flex flex-wrap gap-2">{['Forex', 'Commodity', 'US Indices', 'US Stocks'].map(group => <StatusBadge key={group}>{group}</StatusBadge>)}</div></div>
        <div className="mt-5 rounded-[var(--mos-radius-panel)] border border-amber-500/25 bg-amber-500/10 p-4 text-sm leading-6 text-amber-100"><strong>Verification boundary:</strong> selection uses the publisher headline and summary metadata. Latest Update Time is shown only when the feed provides it. Open every link and read the full article before using the result externally.</div>
      ...
      {result && <section className="space-y-4">
        <Toolbar><SectionHeader title="Selected market news" description={`${result.items.length} selected from ${result.candidateCount} candidates · Model: ${result.model}`} /><Button variant="primary" onClick={() => void download()} disabled={!reviewConfirmed}>Download DOCX</Button></Toolbar>
        ...
        <label className="flex cursor-pointer gap-3 rounded-[var(--mos-radius-panel)] border border-amber-500/25 bg-amber-500/10 p-4 text-sm leading-6 text-amber-100"><input type="checkbox" checked={reviewConfirmed} onChange={event => setReviewConfirmed(event.target.checked)} className="mt-1 h-4 w-4 accent-emerald-500" /><span>I have opened every link, read the full articles, and checked the title, time, main event, factual development, and market relevance. Confirm to enable Download DOCX.</span></label>
      </section>}

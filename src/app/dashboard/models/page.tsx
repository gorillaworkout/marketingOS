...
export default function ModelsPage() {
  ...
  return <PageStack className="max-w-[1440px]">
    <PageHeader eyebrow="AI governance / Learning center" title="Models" description="Learn how each model behaves, compare the fit for each workflow, then set the models that are available and the model you prefer on one page." />
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
      ...
      <Panel padding="compact" className="bg-indigo-400/[0.035]"><p className="text-xs font-medium text-indigo-100">How to use this page</p><p className="mt-1 text-[11px] leading-5 text-[var(--mos-text-muted)]">Choose a workflow to see contextual recommendations. {MODEL_GUIDANCE_DISCLAIMER}</p></Panel>
    </div>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Panel className="border-sky-400/15 bg-sky-400/[0.035]">
        ...
        <h2 className="mt-2 text-base font-semibold text-[var(--mos-text)]">Start with Gemini Flash, move to Claude for copy nuance</h2>
        <p className="mt-2 text-xs leading-5 text-[var(--mos-text-muted)]">Gemini Flash fits fast iteration, hook variations, captions, and image prompts. Claude Sonnet fits better when the priority is a natural tone, storytelling, and copy that reads as if a person wrote it. Pecut Free is still useful for non-urgent experiments, but its latency varies.</p>
      </Panel>
      <Panel className="border-violet-400/15 bg-violet-400/[0.035]">
        ...
        <h2 className="mt-2 text-base font-semibold text-[var(--mos-text)]">Gemini Flash for previews, Claude Sonnet for narration</h2>
        <p className="mt-2 text-xs leading-5 text-[var(--mos-text-muted)]">Gemini Flash is efficient when you need several hooks and preview options. Claude Sonnet is stronger for long voice-over, story flow, and natural transitions. A premium reasoning model is more useful for a complex brief or a review, not as the default for every script.</p>
      </Panel>
    </div>
    ...
    {view === 'library' ? <>
      ...
      <div><p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-indigo-300">Personal scope</p><h2 className="mt-1 text-lg font-semibold text-[var(--mos-text)]">My preferences</h2><p className="mt-1 text-xs leading-5 text-[var(--mos-text-muted)]">Choices here apply only to your account. An administrator still sets the organization default, and that default is the effective model when there is no personal override.</p></div>
      {preferences.length === 0 && <Panel className="border-amber-400/15 bg-amber-400/[0.035]"><h2 className="text-base font-semibold text-[var(--mos-text)]">No workflow preference available yet</h2><p className="mt-2 text-xs leading-5 text-[var(--mos-text-muted)]">You can still study every model in the Model library. Personal choices appear after an administrator enables Social Post, Video Script, or Event Plan for your department.</p></Panel>}

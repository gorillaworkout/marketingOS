    ...
    <PageStack>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
        <div className="rounded-[var(--mos-radius-panel)] border border-[var(--mos-border)] bg-[var(--mos-raised)] p-5 md:p-6">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--mos-text-muted)]">Gunakan flow ini untuk memastikan artikel memiliki sumber aktual, query yang tervalidasi, struktur SEO yang terarah, dan orisinalitas yang lolos sebelum publikasi.</p>
      ...
      <section aria-labelledby="flow-title">
        <div className="mb-4 flex items-end justify-between gap-4">
          ...
          <span className="hidden rounded-full bg-[var(--mos-raised)] px-3 py-1 text-xs text-[var(--mos-text-muted)] sm:block">Baca dari kiri ke kanan, lalu ke baris berikutnya</span>
        </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-[var(--mos-radius-panel)] border border-amber-500/25 bg-amber-500/5 p-5 md:p-6">
          ...
          <div className="mt-4 space-y-3">
            <div className="rounded-[var(--mos-radius-panel)] border border-[var(--mos-border)] bg-[var(--mos-surface)] p-4 text-sm text-[var(--mos-text-secondary)]"><span className="font-semibold text-white">Investing.com: ada artikel relevan?</span><br />Ya → jadikan referensi, lanjut ke SEO Competitor Research, dan lewati Kontan. Tidak → lanjut ke Kontan.</div>
            <div className="rounded-[var(--mos-radius-panel)] border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-100"><span className="font-semibold">Kontan: masih tidak ada artikel relevan?</span><br />Hentikan penulisan. Ulangi kedua riset setiap <strong>10 menit</strong> sampai ada topik yang valid.</div>
          </div>
        </div>
        <div className="rounded-[var(--mos-radius-panel)] border border-emerald-500/25 bg-emerald-500/5 p-5 md:p-6">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-[var(--mos-radius-panel)] border border-emerald-500/25 bg-emerald-500/10 p-4"><p className="text-2xl font-bold text-emerald-300">&gt;90%</p><p className="mt-1 text-sm text-emerald-100">SmallSEOTools: siap terbit bila seluruh checklist juga lengkap.</p></div>
            <div className="rounded-[var(--mos-radius-panel)] border border-red-500/25 bg-red-500/10 p-4"><p className="text-2xl font-bold text-red-300">&lt;90%</p><p className="mt-1 text-sm text-red-100">Wajib revisi segmen bertanda dan periksa ulang.</p></div>
          </div>
        </div>
      </section>
        ...
        <div className="rounded-[var(--mos-radius-panel)] border border-[var(--mos-border)] bg-[var(--mos-raised)] p-5 md:p-6">
          ...
          <dl className="mt-4 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
            {['Tanggal & waktu riset', 'Sumber referensi', '5 judul + waktu + URL', 'Keyword teridentifikasi', 'SEO H1 & sudut artikel', 'Status: siap / menunggu / revisi', 'Skor plagiarism checker', 'Tautan DOCX / Google Docs'].map(item => (
              <div key={item} className="rounded-lg bg-[var(--mos-surface)] px-3 py-2.5"><dt className="text-[var(--mos-text-faint)]">{item}</dt><dd className="mt-1 text-[var(--mos-text-secondary)]">Diisi oleh operator</dd></div>
            ))}
          </dl>
        </div>
        <div className="rounded-[var(--mos-radius-panel)] border border-[var(--mos-border)] bg-[var(--mos-raised)] p-5 md:p-6">
          ...
          <div className="mt-4 space-y-3">
            <details className="group rounded-[var(--mos-radius-panel)] border border-[var(--mos-border)] bg-[var(--mos-surface)] p-4"><summary className="cursor-pointer list-none font-medium text-white">Standar riset &amp; SEO <span className="float-right text-blue-400 group-open:rotate-45">+</span></summary><div className="mt-3 text-sm leading-6 text-[var(--mos-text-muted)]">Gunakan artikel terkini yang mengandung salah satu keyword fokus. Di Google Incognito, ambil H1/H2/H3 dari lima kompetitor dan lima People Also Ask setelah menerapkan filter 24 jam terakhir.</div></details>
            <details className="group rounded-[var(--mos-radius-panel)] border border-[var(--mos-border)] bg-[var(--mos-surface)] p-4"><summary className="cursor-pointer list-none font-medium text-white">Standar draf artikel <span className="float-right text-blue-400 group-open:rotate-45">+</span></summary><div className="mt-3 text-sm leading-6 text-[var(--mos-text-muted)]">Sertakan keyword di paragraf pertama; gunakan H1/H2/H3 dan lima PAA sebagai heading. Cantumkan outlet serta tanggal sumber. Kutipan analis/institusi hanya boleh dipakai bila data sumber mendukungnya. Akhiri dengan CTA satu kalimat ke akun Dupoin.</div></details>
            <details className="group rounded-[var(--mos-radius-panel)] border border-[var(--mos-border)] bg-[var(--mos-surface)] p-4"><summary className="cursor-pointer list-none font-medium text-white">Prompt pengingat editor <span className="float-right text-blue-400 group-open:rotate-45">+</span></summary><div className="mt-3 text-sm leading-6 text-[var(--mos-text-muted)]">“Tulis secara faktual dan jurnalistik dalam Bahasa Indonesia, tanpa fabrikasi. Pertahankan konteks sumber, hindari penyebutan broker kompetitor, dan pastikan setiap klaim dapat ditelusuri ke data riset.”</div></details>
          </div>
        </div>
      </section>

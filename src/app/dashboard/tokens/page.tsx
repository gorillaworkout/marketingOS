  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-400 mt-1">Track AI consumption by office or personal account.</p>
      </div>
      ...
      <section className="bg-gray-800/50 rounded-xl p-6 border border-gray-700/50">
        <h2 className="text-lg font-semibold text-white mb-4">Usage by Account</h2>
        {data?.accountBreakdown?.length ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {data.accountBreakdown.map((account) => (
              <div key={`${account.account_source}-${account.provider}`} className="rounded-lg border border-gray-700 bg-gray-900/50 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs text-gray-400">{providerNames[account.provider] || account.provider}</span>
                </div>
                <div className="mt-4 text-xl font-semibold text-white">{account.total_tokens.toLocaleString()} tokens</div>
        ...
        ) : <p className="text-gray-500">No account usage data yet.</p>}
      </section>
